...
const hasAgy = isAgyAvailable();

describe("Antigravity Plugin Integration (agy)", () => {
  it(
    "passes agy plugin validate with skills and hooks",
    { skip: !hasAgy ? "agy binary not found in PATH" : false },
    async () => {
  ...
  it(
    "installs and uninstalls cleanly via agy plugin install in isolated environment",
    { skip: !hasAgy ? "agy binary not found in PATH" : false },
    async () => {
